...
function getDefaultSearchRevealOrigin() {
...
function SearchModalShortcutKey({ children }: { children: ReactNode }) {
  return (
    <kbd className="inline-flex min-w-[1.125rem] items-center justify-center rounded border border-zinc-200 bg-zinc-50 px-1 py-px text-[10px] font-medium leading-none text-zinc-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400">
      {children}
    </kbd>
  );
}

function SearchModalFooterAction({
  label,
  children,
}: {
  label: string;
  ...
  return (
    <div className="flex shrink-0 flex-wrap items-center gap-x-5 gap-y-2 rounded-b-[24px] border-t border-zinc-200 bg-white px-4 py-2.5 text-xs text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400">
      <SearchModalFooterAction label="Select">
        ...
        <SearchModalShortcutKey>
          <LuArrowDown className="size-2.5" aria-hidden="true" />
        </SearchModalShortcutKey>
      </SearchModalFooterAction>
      <SearchModalFooterAction label="Open">
        <SearchModalShortcutKey>
          <LuCornerDownLeft className="size-3" aria-hidden="true" />
        </SearchModalShortcutKey>
      </SearchModalFooterAction>
      <SearchModalFooterAction label="Open in New Window">
        <SearchModalShortcutKey>⌘</SearchModalShortcutKey>
        <SearchModalShortcutKey>
          <LuCornerDownLeft className="size-3" aria-hidden="true" />
        </SearchModalShortcutKey>
      </SearchModalFooterAction>
...
export function SearchModal({
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (event.key === "Enter" && activeIndex >= 0) {
        event.preventDefault();
        openActiveTask(event.metaKey || event.ctrlKey);
      }
    }
  ...
  function openTaskInNewWindow(task: SearchTask) {
    const path = buildTodoPath({ kind: "task", taskId: task.id });
  ...
  function moveActiveIndex(direction: 1 | -1) {
    ...
    setActiveIndex((current) => {
      if (current < 0) {
        return direction > 0 ? 0 : results.length - 1;
      }
  ...
  function openActiveTask(inNewWindow: boolean) {
    const task = activeIndex >= 0 ? results[activeIndex] : null;
    if (!task) return;
    ...
    >
      ...
      >
        <div className="shrink-0 border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="h-[35px] w-full bg-transparent text-[16px] text-zinc-900 outline-none placeholder:text-zinc-400 dark:text-zinc-50"
            />
          </div>
          <div
        ...
        <div className="search-modal-body min-h-0 flex-1">
          <div className="search-modal-body-inner flex min-h-0 flex-col">
            <div
                ...
                <div className="flex min-h-0 w-[42%] shrink-0 flex-col overflow-y-auto border-r border-zinc-200 dark:border-zinc-800">
                  ...
                  >
                    {results.map((task, index) => {
                      ...
                      const isCheckAnimating =
                        checkAnimatingTaskIds?.has(task.id) ?? false;
                      ...
                      return (
                        <Fragment key={task.id}>
                          ...
                          <li
                            ...
                            className={`group flex min-h-11 cursor-pointer! items-center gap-3 border-b border-zinc-100 px-4 transition-[filter] duration-150 dark:border-zinc-800 ${
